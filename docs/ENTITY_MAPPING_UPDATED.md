...
| `orders` | Orders | 🟡 단건·일괄 처리 코드 작성 | 완료된 RETAIL 주문 변환·생성·금액 검증·CSV 저장 |
| `order_items` | Order line items | 🟡 주문 처리에 통합 | SKU로 Variation ID 조회, 주문 요청에 항목 포함, 응답 UID 저장 |
...
| `tax_settings` | 직접 연동 제외 | — | 현재 주문의 tax_rate로 세금 규칙 구성 |

상품과 고객의 1차 완료는 기본 생성과 성공 결과 저장을 의미한다.
상품 전체 생성은 앞선 테스트에서 완료했다. 기존 정보 수정과 재실행 안정화는 별도로 확인한다.
매장 Square Location ID 생성은 완료했고, 주문 코드에서는 매장 CSV에서 ID를 조회하도록 작성했다.
주문은 단건 테스트와 일괄 처리 코드 작성까지 반영했다. 실제 생성 응답, 금액 검증 및 전체 대상 저장 성공은 실행 결과 확인 후 기록한다.
...
현재 고객 테스트에서는 새 생성 요청의 키를 `uuid4()`로 만든다.
...
주문의 `store_id`로 매장 CSV의 `store_id`를 찾아 `square_location_id`를 가져오도록 작성했다.
...
주문 코드의 매장 ID 저장 컬럼은 `square_location_id`를 사용한다. 매장 생성 자체의 저장 실패 복구 방식은 별도 확인 항목으로 남긴다.

### Order / Order Item Mapping — 단건·일괄 처리 코드 작성

#### 처리 대상

첨부한 주문 CSV는 36건이고, 주문 항목 CSV는 92행이다.
매장 주문 22건 중 완료 19건, 취소 1건, 환불 2건이다.

현재 코드는 `channel = 'RETAIL'`이면서 `order_status = 'COMPLETED'`인 주문을 선택한다.
변환 함수는 할인·배송비가 없고 수량이 양의 정수인 주문을 처리한다.
첫 단건 테스트 대상은 `CCF-000004`이며, 일괄 처리 대상은 현재 CSV 기준 19건이다.
19건은 **대상 건수**이며 실제 성공 건수는 아직 확인하지 않았다.
...
그림은 CSV의 논리적 관계와 예시 값이며 실제 DB의 자료형·외래 키 제약을 선언한 것은 아니다.

**`order_items.order_id = orders.order_id`로 연결한다. `orders.id`와 연결하지 않는다.**
이번 예시는 Midnight Dark Chocolate Bar 2개를 구매한 주문으로, 항목은 한 행이고 수량은 2다.
첨부 데이터의 `item_count`는 항목 행 수가 아니라 수량 합계와 일치했다.

#### 기존 CSV에서 Square ID 조회

주문 CSV의 Square 컬럼은 처음에는 비어 있으므로, 기존 연동 결과가 있는 매장·고객·상품 CSV에서 가져온다.

| 원본 연결 기준 | 조회 기준 | 가져오는 값 |
|---|---|---|
| `orders.store_id` | `stores.store_id` | `square_location_id` |
| `orders.customer_id` | `customers.id` | `square_customer_id` |
| `order_items.sku` | `products.sku` | `square_catalog_object_id` — Variation ID |

단건 테스트에서는 `.loc[조건, 컬럼].iloc[0]`으로 ID를 꺼내고,
파이프라인에서는 `find_square_id()`로 조회 대상이 한 건인지와 ID가 비어 있지 않은지 확인한다.
SKU별 Variation ID는 `variation_ids_by_sku` 딕셔너리로 전달한다.

- 비회원 주문은 고객 ID를 생략한다.
- 원본 고객 ID가 있는 주문은 Square 고객 ID가 필요하다.
- 온라인 가입 고객의 매장 주문도 같은 조회 규칙을 적용한다. 고객 매핑이 없으면 중단하며 고객을 자동 생성하지 않는다.

#### 요청 필드 매핑

아래 경로는 반환된 payload 기준이다.

| 원본 값 또는 설정 | Square 요청 필드 | 처리 |
|---|---|---|
| `orders.order_id` | `order.reference_id` | 원본 주문 번호 |
| 조회한 매장 ID | `order.location_id` | Square Location ID |
| 조회한 고객 ID | `order.customer_id` | 회원 주문에서 연결 |
| `order_items.id` | `order.line_items[].uid` | 원본 항목 UUID |
| SKU로 조회한 Variation ID | `order.line_items[].catalog_object_id` | 상품 판매 단위 연결 |
| `order_items.quantity` | `order.line_items[].quantity` | `2` → `"2"` |
| `order_items.unit_price` | `order.line_items[].base_price_money.amount` | `7.25` → `725`센트 |
| 통화 설정 | `order.line_items[].base_price_money.currency` | `CAD` |
| `orders.tax_rate` | `order.taxes[].percentage` | `0.0500` → `"5"` |
| 생성 상태 설정 | `order.state` | `OPEN` |
| 요청 키 | `idempotency_key` | 주문별 고정 키 |

세율이 0보다 크면 `uid = source-order-tax`, `name = Sales Tax`, `type = ADDITIVE`, `scope = ORDER`를 함께 구성한다.
`pricing_options`의 `auto_apply_taxes`, `auto_apply_discounts`는 모두 `False`로 지정한다.
원본 세금과 총액은 응답 검증에 사용하며 요청의 총액으로 직접 지정하지 않는다.

#### 작성한 실행 흐름

```mermaid
flowchart TD
    A[완료된 RETAIL 주문 선택] --> B{기존 SUCCESS와 주문 ID 존재?}
    B -->|예| C[건너뛰기]
    B -->|아니오| D[항목과 Square ID 연결]
    D --> E[transform_order로 검증·변환]
    E --> F[기존 요청 비교 및 요청 보관]
    F --> G{보관한 응답 존재?}
    G -->|예| H[응답 재사용]
    G -->|아니오| I[create_order 호출 및 응답 보관]
    H --> J[응답 연결 정보와 금액 검증]
    I --> J
    J --> K[항목 CSV 저장 후 주문 CSV 저장]
```

`transform_order()`는 주문·항목 연결, 매장 일치, 항목 ID 중복, 수량·금액을 확인하고 payload를 반환한다.
`create_order()`는 Sandbox의 주문 생성 API를 호출한 뒤 응답의 `order` 객체를 반환한다.
따라서 생성된 ID는 `square_order["id"]`로 읽는다.
파이프라인은 한 주문에서 오류가 발생하면 중단한다.

#### 금액 검증

금액은 `Decimal`로 변환한 뒤 센트 정수로 비교한다.
현재 `to_cents()`는 음수나 센트 미만 단위를 임의로 반올림하지 않고 오류로 처리한다.

1. 항목별 `quantity × unit_price = line_total` 확인
2. 항목 합계와 원본 `subtotal` 비교
3. 원본 `subtotal + tax_amount = total_amount` 확인 — 할인·배송비 0인 현재 범위
4. API 응답의 세금·총액과 원본 금액 비교

`CCF-000004`의 원본 소계는 1450센트, 세금은 73센트, 총액은 1523센트다.
실제 Square 응답은 별도로 확인해야 한다. 반올림 방식 등으로 금액이 다르면 생성된 주문 ID를 유지하고 원인을 확인한다.

#### 주문과 항목 CSV 저장

| 파일 | 업데이트 컬럼 |
|---|---|
| `orders_export.csv` | `square_order_id`, `square_customer_id`, `square_location_id`, `square_sync_status`, `square_synced_at`, `square_sync_error` |
| `order_items_export.csv` | `square_line_item_uid`, `square_catalog_object_id` |

주문 상태는 `SUCCESS`, 저장 시각은 UTC로 기록하고 성공 시 오류 컬럼을 비운다.
항목은 원본 항목 ID와 응답 `uid`를 비교해 연결하며, 전체 CSV에서 해당 주문의 행만 갱신한다.
원본 `order_status`는 변경하지 않는다.

각 CSV는 `.csv.tmp`에 먼저 쓴 다음 원본 경로로 교체한다.
두 파일이 하나의 트랜잭션으로 저장되는 것은 아니므로, 항목 저장 후 주문 저장 실패가 발생할 수 있다.
생성된 Square 주문은 유지하고 같은 응답으로 저장을 재시도한다.
실패 행의 `FAILED` 상태·오류 메시지를 CSV에 별도 기록하는 기능은 현재 미구현이다.

#### 재실행과 복구

주문 키는 고객 테스트의 `uuid4()`와 달리 다음 규칙으로 유지한다.

```text
ccf-sandbox-order-CCF-000004-v1
```

| 보관 파일 | 역할 |
|---|---|
| `data/processed/<order_id>_request.json` | 동일 키의 요청 내용 비교 및 재시도 |
| `data/processed/<order_id>_response.json` | 생성 결과 검증 및 CSV 저장 복구 |

- 기존 요청과 payload가 다르면 자동 전송하지 않는다.
- 응답 파일만 있고 요청 파일이 없으면 중단한다.
- 응답이 있으면 API 재호출 대신 검증·저장을 재시도한다.
- 응답이 없으면 동일 키·동일 내용으로 생성 요청을 보낸다.
- 생성 후 응답 파일 저장에 실패해도 새 키로 즉시 재생성하지 않는다.

보관한 응답 사용은 현재 생성 테스트의 복구 방식이다.
향후 변경·결제·환불을 동기화할 때에는 최신 Square 주문을 조회하는 흐름이 필요하다.

#### 완료 상태의 의미

| 상태 | 의미 |
|---|---|
| 원본 `order_status = COMPLETED` | 원본 시스템의 거래 상태 |
| Square `state = OPEN` | 이번 코드가 생성하는 주문 상태 |
| 로컬 `square_sync_status = SUCCESS` | 생성·금액 검증·CSV 저장 성공 |

주문 생성 성공은 결제 완료를 의미하지 않는다.
할인·배송비·취소·환불·리워드 사용은 현재 변환 범위에 포함하지 않았다.
...
| `src/transform/orders.py` | 주문·항목 검증 및 Square payload 변환 |
| `src/square/orders.py` | Square 주문 생성 API 호출 |
| `tests/test_orders.py` | CCF-000004 선택, ID 조회, 단건 생성·검증·저장 코드 |
| `src/pipelines/order_pipeline.py` | 완료된 RETAIL 주문 일괄 처리·기존 성공 건너뛰기·응답 기반 복구 |
...
원본 주문 UUID, 사람이 보는 주문 번호, Square 주문 ID는 서로 다른 값이다.
주문 항목은 실제 CSV의 `order_id`로 연결하며, API 응답과는 항목 `uid`로 연결한다.
원본 주문 완료·Square 주문 상태·CSV sync 성공은 각각 따로 확인한다.

### 원본 거래 시각과 복제 시각을 구분한다

과거 주문을 Square에 새로 생성하면 Square 생성 시각은 원본 거래 시각과 다르다.
이후 날짜별 매출 분석을 위해 원본 `created_at`과 주문 ID 매핑을 보존해야 한다.
...
1. **단건 실행 확인** — 작성한 CCF-000004 테스트에서 ID 연결, 실제 API 응답, 세금·총액 일치를 확인한다.
2. **저장과 복구 확인** — 주문·항목 CSV 저장 결과와 같은 요청 재실행을 확인한다.
3. **일괄 실행 확인** — 작성한 `order_pipeline.py`로 완료된 RETAIL 주문을 처리하고 대상·성공·건너뜀 건수를 기록한다.
4. **Sandbox 결제 구현** — 생성한 Square 주문과 결제를 연결한다.
5. **거래 범위 확장** — 취소·환불·할인·리워드 사용 등 추가 시나리오를 처리한다.
6. **분석용 수집으로 이동** — Square 매장 데이터와 Lovable 온라인 데이터를 GCP·BigQuery로 수집한다.
7. **모델링과 자동화** — Zoomcamp 학습 내용을 적용해 통합 모델과 정기 배치 실행을 구성한다.

현재까지는 **분석에 사용할 가상의 매장 데이터 소스를 준비하는 단계**다.
주문 단건·일괄 처리 코드는 작성했으며, 실제 전체 연동 성공은 실행 결과 확인 후 기록한다.
결제 처리와 GCP 적재는 아직 완료 항목에 포함하지 않는다.
